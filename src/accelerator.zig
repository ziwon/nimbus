...
fn parseNvidiaSmiCsvWithCdi(
    ...
    while (lines.next()) |raw_line| {
        ...
        defer for (fields) |field| allocator.free(field);
...
fn parseCsvLine(allocator: std.mem.Allocator, line: []const u8) ![6][]const u8 {
    ...
    errdefer for (result[0..owned_count]) |field| allocator.free(field);
    var index: usize = 0;

    while (owned_count < result.len) {
        const field_index = owned_count;
        ...
        if (index < line.len and line[index] == '"') {
            ...
            while (index < line.len and (line[index] == ' ' or line[index] == '\t')) index += 1;
            result[field_index] = try field.toOwnedSlice(allocator);
        } else {
            const start = index;
            ...
            result[field_index] = try allocator.dupe(u8, value);
        }
        owned_count += 1;

        if (owned_count < result.len) {
            if (index >= line.len or line[index] != ',') return error.MalformedProviderOutput;
            index += 1;
...
test "nvidia CSV parser handles quoted fields and optional values" {
    ...
    defer collector.deinit();
    const output =
        "0, GPU-aaaa, \"NVIDIA A100, PCIe\", 40960, 550.54.15, 8.0\n" ++
